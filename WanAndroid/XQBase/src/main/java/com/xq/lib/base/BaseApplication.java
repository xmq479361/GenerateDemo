...
public class BaseApplication extends Application {
  ...
  void initLog() {
    LogUtils.Config config = LogUtils.getConfig()
        ...
        .setStackOffset(2)// 设置栈偏移，比如二次封装的话就需要设置，默认为 0
        .setSaveDays(3)// 设置日志可保留天数，默认为 -1 表示无限时长
        // 新增 ArrayList 格式化器，默认已支持 Array, Throwable, Bundle, Intent 的格式化输出
        .addFormatter(new LogUtils.IFormatter<ArrayList<Object>>() {
          @Override
          public String format(ArrayList<Object> list) {
  ...
  public boolean isDebug(){
    return isDebug =  AppUtils.isAppDebug();
  }

  public void execute(Runnable runnable) {
    if(null == cachedPool) {
      cachedPool = ThreadUtils.getCachedPool();
    }
